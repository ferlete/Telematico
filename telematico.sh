...
# Telematico, Um Bot do Telegram escrito em bash.
# Escrito por Ten Ferlete (ferlete@gmail.com).
# https://github.com/ferlete/Telematico


if [ ! -f "JSON.sh/JSON.sh" ]; then
	echo "Você não clonou recursivamente! Carregando JSON.sh..."
	git clone http://github.com/dominictarr/JSON.sh
	echo "JSON.sh foi baixado. Processo."
fi

if [ ! -f "token" ]; then
	clear
	echo -e '\e[0;31mTOKEN FALTANDO.\e[0m'
	echo "POR FAVOR ESCREVA SEU TOKEN AQUI"
	read token
	echo "$token" >> token
fi
...
# digitando para mensagens de texto, upload_photo para fotos, record_video ou upload_video para vídeos, record_audio ou upload_audio para arquivos de áudio, upload_document para arquivos gerais, find_location para localização
...
case "$1" in
	...
	*)
		...
		echo -e '\e[0;31mArgumentos Disponíveis: outproc, count, broadcast, start, kill, help, attach\e[0m'
		;;
esac
